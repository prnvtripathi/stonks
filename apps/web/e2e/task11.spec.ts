...
test("explains a persisted match, preserves back navigation, and sends server sort requests", async ({ page }) => {
  ...
  await page.route("**/api/v1/screens/volume-breakout/runs**", (route) => route.fulfill({ json: { screen, runs: [{ id: "run-1", screenId: screen.id, datasetId: "dataset-1", effectiveDate: "2026-09-04", completedAt: "2026-09-04T10:00:00Z", matchCount: 1, status: "complete", matches: [{ instrumentId: "INFY", entered: true, exited: false }] }] } }));
  await page.route("**/api/v1/screens/volume-breakout/results**", (route) => { resultRequests.push(route.request().url()); return route.fulfill({ json: { screen, run: { id: "run-1", screenId: screen.id, datasetId: "dataset-1", effectiveDate: "2026-09-04", completedAt: "2026-09-04T10:00:00Z", matchCount: 1, status: "complete", matches: [match] }, pagination: { limit: 25, offset: 0, total: 1 } } }); });
