...
export class MemoryResearchStore implements ResearchStore {
  ...
  public async runScreen(datasetId: string, screen: SavedScreen, effectiveDate: string): Promise<RunDetail> {
    ...
    const currentMatches: RunMatch[] = matches.map((item, index) => { const explanation = buildExplanation(screen.source, item); const score = item.metricRows?.find((row) => row.metric === "momentum_score")?.value ?? item.metrics?.momentum_score ?? null; return { instrumentId: item.instrumentId, rank: index + 1, score, explanation, ...(explanation.momentum ? { momentum: explanation.momentum } : {}), entered: !previousIds.has(item.instrumentId), exited: false }; });
...
function buildExplanation(source: string, item: InstrumentRow): Explanation {
  const metrics = item.metricRows?.filter((row) => !row.metric.startsWith("momentum_")).map((row) => row.metric) ?? Object.keys(item.metrics ?? {});
  const clauses: ExplanationClause[] = metrics.slice(0, 8).map((metric) => {
    ...
    return { clause: metric, metric, result: rawValue.state === "not_applicable" ? "Not applicable" : rawValue.state === "missing" ? "Unavailable" : "Matched", value: rawValue };
  });
...
function buildMomentum(item: InstrumentRow): MomentumBreakdown {
  ...
  const components = rows.map((row) => { const id = row.metric.slice("momentum_".length); const metadata = parseMetadata((row as MetricRow & { metadata?: unknown }).metadata); const normalized = typeof metadata.normalized === "number" ? metadata.normalized : row.normalizedValue ?? null; const weight = typeof metadata.weight === "number" ? metadata.weight : 0; const contribution = typeof metadata.contribution === "number" ? metadata.contribution : normalized === null ? null : normalized * weight; return { label: labels[id] ?? id, raw: row.value, normalized, weight, contribution }; });
