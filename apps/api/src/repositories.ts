...
export interface MetricRow { readonly metric: string; readonly value: number | null; readonly state: "present" | "missing" | "not_applicable"; readonly effectiveDate?: string | null; readonly stale?: boolean; readonly rawValue?: string | null; readonly normalizedValue?: number | null; readonly formulaVersion?: string | null; readonly sourceArtifactId?: string | null; }
...
export interface ExplanationClause { readonly clause: string; readonly result: "Matched" | "Unavailable" | "Not applicable" | "Not matched"; readonly value?: Omit<MetricRow, "metric">; }
export interface MomentumComponent { readonly label: string; readonly raw: number | null; readonly normalized: number | null; readonly weight: number; readonly contribution: number | null; }
...
export class MemoryResearchStore implements ResearchStore {
  ...
  public async runScreen(datasetId: string, screen: SavedScreen, effectiveDate: string): Promise<RunDetail> {
    ...
    const currentMatches: RunMatch[] = matches.map((item, index) => { const explanation = buildExplanation(screen.source, item); return { instrumentId: item.instrumentId, rank: index + 1, score: item.metrics?.momentum_score ?? null, explanation, ...(explanation.momentum ? { momentum: explanation.momentum } : {}), entered: !previousIds.has(item.instrumentId), exited: false }; });
...
export class D1ResearchStore implements ResearchStore {
  ...
  public async runScreen(datasetId: string, screen: SavedScreen, effectiveDate: string): Promise<RunDetail> {
    ...
    const currentMatches: RunMatch[] = rows.results.map((row, index) => ({ instrumentId: row.instrument_id, rank: index + 1, score: row.score == null ? null : Number(row.score), explanation: { matched: true, text: `Matched ${screen.source}`, metrics: compiled.referencedMetricIds, clauses: [{ clause: screen.source, result: "Matched" }] }, entered: !priorIds.has(row.instrument_id), exited: false }));
...
function buildExplanation(source: string, item: InstrumentRow): Explanation {
  const metrics = Object.keys(item.metrics ?? {});
  const clauses: ExplanationClause[] = metrics.slice(0, 8).map((metric) => {
    const row = item.metricRows?.find((candidate) => candidate.metric === metric);
    const rawValue = row ?? { value: item.metrics?.[metric] ?? null, state: item.metrics?.[metric] == null ? "missing" as const : "present" as const };
    return { clause: metric, result: rawValue.state === "not_applicable" ? "Not applicable" : rawValue.state === "missing" ? "Unavailable" : "Matched", value: rawValue };
  });
  const definitions: readonly [string, number][] = [["Weighted 12-month RS percentile", .35], ["Six-month performance", .2], ["Three-month performance", .15], ["Trend strength vs moving averages", .1], ["Proximity to 52-week high", .1], ["Volume confirmation", .1]];
  const components: MomentumComponent[] = definitions.map(([label, weight]) => {
    const key = label.startsWith("Weighted") ? "rs_rating" : label.startsWith("Six") ? "return_6m" : label.startsWith("Three") ? "return_3m" : label.startsWith("Trend") ? "trend_strength" : label.startsWith("Proximity") ? "high_52w_proximity" : "volume_confirmation";
    const raw = item.metrics?.[key] ?? null;
    const normalized = raw == null ? null : Math.max(0, Math.min(100, raw));
    return { label, raw, normalized, weight, contribution: normalized == null ? null : normalized * weight };
  });
  const available = components.filter((component) => component.normalized !== null).length;
  const sourceDate = item.metricRows?.find((row) => row.effectiveDate)?.effectiveDate ?? null;
  const momentum: MomentumBreakdown = { components, cohort: item.assetClass === "equity" ? "NSE EQ ordinary shares" : item.assetClass === "etf" ? "NSE ETFs" : "AMFI mutual-fund category", formulaVersion: "momentum-v1", coverage: components.length ? available / components.length : 0, sourceDate, ...(available < 4 ? { warning: "Insufficient component coverage; score may be unavailable." } : {}) };
  return { matched: true, text: `Matched ${source}`, metrics, clauses, momentum };
}
