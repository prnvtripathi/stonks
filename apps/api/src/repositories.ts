...
export interface ResearchStore {
  ...
  runScreen(datasetId: string, screen: SavedScreen, effectiveDate: string, completedAt?: string): Promise<RunDetail>;
...
export class MemoryResearchStore implements ResearchStore {
  ...
  public async runScreen(datasetId: string, screen: SavedScreen, effectiveDate: string, completedAt = new Date().toISOString()): Promise<RunDetail> {
    ...
    const currentMatches: RunMatch[] = matches.map((item, index) => { const explanation = buildExplanation(screen.source, item, parsed.value!); const score = item.metricRows?.find((row) => row.metric === "momentum_score")?.value ?? item.metrics?.momentum_score ?? null; return { instrumentId: item.instrumentId, rank: index + 1, score, symbol: item.symbol, name: item.name, assetClass: item.assetClass, explanation, ...(explanation.momentum ? { momentum: explanation.momentum } : {}), entered: !previousIds.has(item.instrumentId), exited: false }; });
    ...
    const detail: RunDetail = { id: crypto.randomUUID(), screenId: screen.id, datasetId, effectiveDate, completedAt, matchCount: matches.length, status: "complete", source: screen.source, languageVersion: screen.languageVersion, matches: [...currentMatches, ...exitedMatches] };
...
export class D1ResearchStore implements ResearchStore {
  ...
  public async runScreen(datasetId: string, screen: SavedScreen, effectiveDate: string, completedAt = new Date().toISOString()): Promise<RunDetail> {
    ...
    const matchStatements = matches.map((match, index) => this.db.prepare("INSERT INTO screen_matches (dataset_id, run_id, ordinal, instrument_id, score, symbol, name, asset_class, explanation_json, entered, exited) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(datasetId, runId, index + 1, match.instrumentId, match.score, match.symbol, match.name, match.assetClass, JSON.stringify(match.explanation), match.entered ? 1 : 0, match.exited ? 1 : 0));
    const runStatement = this.db.prepare("INSERT INTO screen_runs (dataset_id, run_id, screen_id, effective_date, status, result_count, query_version, source, language_version, completed_at) VALUES (?, ?, ?, ?, 'complete', ?, ?, ?, ?, ?)").bind(datasetId, runId, screen.id, effectiveDate, rows.results.length, screen.languageVersion, screen.source, screen.languageVersion, completedAt);
    if (this.db.batch) {
    ...
    return { id: runId, screenId: screen.id, datasetId, effectiveDate, completedAt, matchCount: rows.results.length, status: "complete", source: screen.source, languageVersion: screen.languageVersion, matches };
  }
