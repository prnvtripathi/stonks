...
- Fix round 1 added unit checking. Metric units are stored as numeric percentage points (for example `3%` binds `3`, not `0.03`); bare numeric literals are dimensionless thresholds. Incompatible units such as `Volume > 10%` are rejected, while `Return over 1day > 3%` is valid.
