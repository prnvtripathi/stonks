...
| R02: fingerprint every contributing input and preserve lineage | F03 | `9f102c3` | `HEAD` | `.venv/bin/python -m pytest pipeline/tests/integration/test_input_identity.py pipeline/tests/integration/test_candidate_gate.py pipeline/tests/integration/test_publication.py -q` (33 passed); `.venv/bin/python -m pytest pipeline/tests -q` (211 passed); `git diff --check` | self-review complete | None identified | DONE |
