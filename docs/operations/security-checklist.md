...
| `ACCESS_ALLOWED_SERVICE_TOKENS` | Worker secret, per environment (`wrangler secret put ACCESS_ALLOWED_SERVICE_TOKENS --env <env>`) | The `common_name` of each CI Service Token allowed to smoke-test `GET /api/v1/status` (see R12/F15 in `deploy.md`). Rotate whenever the corresponding `CF_ACCESS_SERVICE_TOKEN_ID`/`_SECRET` pair below is regenerated -- the `common_name` allowlisted here must keep matching the live token's Zero Trust configuration. |
