...
| Match identity survives across dataset changes | `apps/api/src` immutable-saved-run tests (R05) plus `test_selective_restore_reimports_projection_and_preserves_post_promotion_product_state`'s `screen_matches` non-interference assertions | `apps/api/src/index.test.ts`, `test_remote_restore.py` |
